...
const $s365=(function() {
	...
	const teamFilter=
		[
		/* 00 - 19 */ "monchengladbach",null/*BL Konferenz*/,"dortmund","bayern","fortuna","schalke","fckoln","paderborn","leverkusen","leipzig","frankfurt","freiburg","bremen","hertha","union","hoffenheim","wolfsburg","mainz","augsburg",null/*BL*/,
				...
				return new Promise(function (resolve, reject) {					
					if (force!==true && xKey!==null) {
						...
						fetchDoc("http://www.sport365.live/de/home").then((doc)=>{
							...
							console.warn("[209]",e);
							reject(_nw===nw?"Fehler beim Laden des Schlüssels [209]":null);
						}); // fetchDec (catch)
					} // else
				}); // return Promise
			}, // function get
	...
	const getEvents=(function() {
		let nw=null;
		return function (fb,de,callback,err) {
			const _nw=nw=Date.now(),
			_getEvents=function(resolve, reject) {
				XKey.get().then(xKey=>{
					if (_nw===nw) {
						const xUrl="http://www.sport365.live/de/events/-/-/"+(fb?"10":"-")+"/"+(de?"7":"-")+"/"+((new Date).getTimezoneOffset()*-1); // TODO: CHECK !!!!!!!!!! *-1
						console.log("Laden",xUrl);
						fetchDoc(xUrl).then(doc=>{
							if (_nw===nw) {
								...
								let X=[];
								for (let i=0, tr, tr2, url, Idx=[]; tr=trs[i], tr2=trs[i+1]; i+=2) {
									...
									if (x.length===3) {
										try {
											...
											XKey.clear();
											reject("Fehler bei Entschlüsselung der URLs");
											return;
										}
									} // if (x.length===3)
									if (url) {
										...
										if (title!=="") {
											...
											if (idx===-1) {
												...
												X.push(ident({
													...
													title:title,
													sub:sub,
													i:X.length, // um auf Tizen stabil zu sortieren
													online:online,
													de:langDe,
													en:langEn,
													id:z,
													langs:[{lang:lang,online:online,url:url,de:langDe,en:langEn,id:0}]
												}));
											} else {
											...
											// title : Titel
											// sub : Subtitel
											// online : true/false = mindestens ein Stream online
											// de : true/false = Deutsche Sprache vorhanden
											// cat : 0=Bundesliga, 1=2.Bundesliga, (2=-), 3=DFB Pokal, 4=CL, 5=EL, 6=Fussball, (7=-), 8=Sonstiges(Sport gesetzt), 9=Sonstiges(Sport nicht gesetzt)
											// langs: [{ ...}, ... ] einzele Streams
											//		lang : Kommentarsprache (String)		
											//		online : true/false = Stream online 
											//		de : true/false = Deutsche Sprache
											//		url : url
											//
											// fallweise auch:
											// sport : Sportart
											// region : Region (z.B. "Germany", "Europe" oder "World")
											// nat: true/false = deutscher Wettbewerb 													
											// league : Liga/Wettbewerb
											// home : ID/Nummer des Heimteam bzw. der Konferenz / Liga
											// away : ID/Nummer des Auswärtsteams bzw. der Liga											
											
										}	// if (title!=="")
									} // if (url)
								}	// for
								
								X.forEach(e=>e.langs.sort((a,b)=>{
...
//								});
						...
						}).catch(e=>reject(_nw===nw?e:null));
					} else reject(null);
				}).catch(e=>reject(_nw===nw?e:null)); //XKey.get
			};
	...
	const getLinks=function(url,callback,err) {
		const _getLinks=(resolve,reject)=>{
			XKey.get().then(xKey=>{
				...
				fetchDoc("http://www.sport365.live"+url).then(doc=>{
					Z=[];
					for (let i=0, spns=doc.querySelectorAll('span#span_link_links'), spn, x; spn=spns[i]; i++) {
						...
						try {
							...
							console.log("Fehler Decrypt",e);
							XKey.clear();
							reject("Fehler beim Entschlüsseln der URLs");
							return;
						}					
					}
								...
								if (zz) {
									...
									fetchDoc(zz).then(doc=>{
										if (nw===_nw) {
											...
											} else reject("Nichts gefunden [E7]");
										} else reject(null);				
									}).catch(e=>reject(nw===_nw?e:null));
								} else reject("Nichts gefunden [E81]");
							} else reject(null);
						}).catch(e=>reject(nw===_nw?e:null));
					} else reject(null);
				}).catch(e=>reject(nw===_nw?e:null));
	...
	const adshell=(function(){
		let _timer=null, //[], // <-------------------------- für Funktion mehrere "adshell" parallel laufen zu lassen (z.B. 
				_err=0; //[];     // <--------------------------- für "Multi-Stream") _timer und _err als Arrays und 
		const fetchBanner=function(/*id,*/url,callback) {  // <---- "id" hier und im Folgenden entkommentieren
						console.log("Lade adshell (1)"/*,id*/,url);
						fetchTxt(url).then(txt=>{
							const m=txt.match(/url\s*:\s*["']([^"']+)/i);
							if (m) {
								console.log("Lade adshell (2)"/*,id*/,m[1]);
								fetchDoc(m[1]).then(()=>callback(ok(/*id*/))).catch(()=>callback(err(/*id,*/3)));							
								/* wohl nicht notwendig * /
								fetchDoc(m[1]).then(function(doc){
									...
									if (z) {
										//console.log("Lade adshell (3)",id,z[1]);
										console.log("Lade adshell (3)",z[1]);
										//xmlRequest(z[1],{responseType:'document'},()=>callback(ok(id)),()=>callback(err(id,5)));
										xmlRequest(z[1],{responseType:'document'},()=>callback(ok()),()=>callback(err(5)));
									} else callback(err(4));
								}).catch(()=>callback(err(/*id,* /3)));
								/**/
							} else callback(err(/*id,*/2));
						}).catch(e=>callback(err(/*id,*/1)));
					},
					err=function(/*id,*/status) {
						_err/*[id]*/++;
						console.log("Banner-Error: "+status+" ("+_err+")"/*,id*/);
						if (_err/*[id]*/>4 && _timer/*[id]*/!==null) {
							console.log("==> Remove Adshell"/*,id*/);
		...
		return {
			...
			/* für Funktion mit mehreren "adshell"
			clear:function() {
				_timer.forEach((t,i)=>{
					if (t!==null) {
						console.log("Remove Adshell",i);
						clearTimeout(t);
						t=null;					
					}
				});
			...
			*/
			attach:function(/*id,*/banner,callback) {
				const _attach=function(resolve){
					console.log("Attach Adshell"/*,id*/,banner);
					if (_timer/*[id]*/!==null) clearTimeout(_timer/*[id]*/);
					_err/*[id]*/=0;
					if (typeof banner==='string') {
						_timer/*[id]*/=setInterval(fetchBanner,900000/*,id*/,banner,()=>{}); // <----- alle 15 Minuten erneuern
						fetchBanner(/*id,*/banner,resolve);
					} else {
						_timer/*[id]*/=null;
						resolve(-1);
					}
				};
				if (typeof callback==='function') _attach(callback);
						else return new Promise(_attach);
			}
		};
	})(); // adshell
...
//////// FUNKTIONEN /////////
	...
	function fetchTxt(url,o) {
		return fetch(url,o)
				.then(response=>response.text().then(txt=>{
					if (response.ok) {
						return txt;
					} else {
						return Promise.reject("fetch error ("+response.status+")");
					}	
				}));		
	} // fetchTxt
	function fetchDoc(url,o) {
